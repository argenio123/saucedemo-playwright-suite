...
test.describe('Shopping cart', () => {
  ...
  test('TC-027 Adding one product sets the cart badge to 1', async ({ shopper }, testInfo) => {
  ...
  test('TC-028 Adding every product sets the cart badge to 6', async ({ shopper }, testInfo) => {
  ...
  test('TC-029 Removing a product from the catalogue decrements the badge', async ({ shopper }, testInfo) => {
    await shopper.addToCart('Sauce Labs Backpack');
  ...
  test('TC-030 The cart lists exactly the products that were added', async ({ shopper, cartPage }, testInfo) => {
  ...
  test('TC-031 Each distinct product appears once with a quantity of 1', async ({ shopper, cartPage }, testInfo) => {
    await shopper.addToCart('Sauce Labs Backpack');
    await shopper.addToCart('Sauce Labs Bike Light');
    await shopper.openCart();
  ...
  test('TC-032 Removing from the cart page empties the cart', async ({ shopper, cartPage }, testInfo) => {
    await shopper.addToCart('Sauce Labs Backpack');
    await shopper.openCart();
  ...
  test('TC-033 Continue shopping returns to the catalogue with the cart intact', async ({ shopper, cartPage }, testInfo) => {
  ...
  test('TC-034 Cart contents survive a page reload', async ({ shopper, page }, testInfo) => {
    await shopper.addToCart('Sauce Labs Backpack');
  ...
  test('TC-035 Reset App State clears the cart', async ({ shopper }, testInfo) => {
    await shopper.addToCart('Sauce Labs Backpack');
    await shopper.addToCart('Sauce Labs Bike Light');
    await expect(shopper.cartBadge).toHaveText('2');
  ...
  test('TC-036 A product can be added from its detail page', async ({ shopper, page, cartPage }, testInfo) => {
